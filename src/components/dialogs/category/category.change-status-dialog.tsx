...
export function CategoryChangeStatusDialog({ currentCategory, open, onOpenChange, active = true }: Props) {
  const queryClient = useQueryClient();
  const { setCurrentCategory } = useCategoryDialog();
      ...
      <AlertDialogContent className="sm:max-w-[440px] p-0 overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
        ...
        <AlertDialogFooter className="p-4 pt-0 flex sm:justify-end gap-2">
          ...
          >
            ...
            ) : isActive ? "Kích hoạt danh mục" : "Vô hiệu hóa danh mục"}
          </Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
