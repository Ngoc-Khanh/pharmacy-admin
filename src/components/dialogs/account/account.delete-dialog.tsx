...
export const AccountDeleteDialog = memo(function AccountDeleteDialog({ currentAccount, open, onOpenChange }: Props) {
  const queryClient = useQueryClient();
  const [value, setValue] = useState("");
  const isConfirmValid = value.trim() === currentAccount.email;

  const deleteUserMutation = useMutation({
    mutationFn: (id: string) => AccountAPI.AccountDelete(id),
    onSuccess: () => {
      toast.success(`Người dùng ${currentAccount.firstname} ${currentAccount.lastname} đã được xóa thành công.`);
      onOpenChange(false);
  ...
  const handleDelete = () => {
    ...
    deleteUserMutation.mutate(currentAccount.id || "");
  };
        ...
        <div className="space-y-5">
          ...
          >
            <Label className="mb-2 block font-medium">
              Vui lòng nhập <span className="font-mono text-rose-600 dark:text-rose-500">{currentAccount.email}</span> để xác nhận:
            </Label>
