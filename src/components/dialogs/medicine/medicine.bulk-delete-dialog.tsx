...
export function MedicineBulkDeleteDialog({
  open,
  onOpenChange,
  selectedMedicines,
  onSuccess
  ...
  const deleteMutation = useMutation({
    mutationFn: async (medicineIds: string[]) => {
      return await MedicineAPI.MedicineBulkDelete(medicineIds);
    },
    onSuccess: (result) => {
  ...
  const handleConfirm = () => {
    ...
    deleteMutation.mutate(medicineIds);
  };
          ...
          <div className="flex items-center gap-3 p-3 bg-rose-50 dark:bg-rose-900/20 rounded-lg border border-rose-200 dark:border-rose-800/30">
            ...
            <div>
              <p className="font-medium text-rose-800 dark:text-rose-300">
                Bạn có chắc muốn xóa {selectedMedicines.length} dược phẩm đã chọn?
              </p>
