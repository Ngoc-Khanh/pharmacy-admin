...
import { MedicineResponse, PaginationProps } from "@/data/interfaces";
...
interface DataTableProps {
  columns: ColumnDef<MedicineResponse>[];
...
export function MedicineDataTable({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-fuchsia-100 dark:border-fuchsia-800/30 bg-white dark:bg-slate-950 shadow-sm">
        <motion.div
          initial={{ opacity: 0.7 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <div className="overflow-x-auto">
            <Table className="w-full table-auto">
              <TableHeader className="bg-fuchsia-50/80 dark:bg-fuchsia-950/40 sticky top-0 z-10">
                {table.getHeaderGroups().map((headerGroup) => (
                  <TableRow key={headerGroup.id} className="border-b border-fuchsia-100 dark:border-fuchsia-800/20">
                    {headerGroup.headers.map((header) => {
                      return (
                        <TableHead
                          ...
                          className={cn(
                            "h-9 font-medium text-fuchsia-800 dark:text-fuchsia-300 text-sm px-3 py-2 text-left",
                ...
                {table.getRowModel().rows?.length > 0 ? (
                  table.getRowModel().rows.map((row, i) => (
                    <motion.tr
                      ...
                      className="group border-b border-fuchsia-50 dark:border-fuchsia-800/10 hover:bg-fuchsia-50/70 dark:hover:bg-fuchsia-900/20 data-[state=selected]:bg-fuchsia-100 dark:data-[state=selected]:bg-fuchsia-800/30 transition-colors"
                    ...
                    >
                      {row.getVisibleCells().map((cell) => (
                        <TableCell
                          key={cell.id}
                          className={cn(
                            "h-12 px-3 py-2 align-middle text-left",
